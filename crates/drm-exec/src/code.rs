...
pub fn evolve_task(work: &Path, manifest: &Path) -> Result<MutationReport, ExecError> {
    ...
    if source.len() > 262_144 {
        return Err(ExecError::CodeDenied("source exceeds 262144 bytes".into()));
    }
    ...
    while score < task.cases.len() && evaluated < task.max_candidates {
        ...
        for candidate in derive_candidates(&source) {
            if evaluated >= task.max_candidates || !seen.insert(candidate.clone()) {
                continue;
            }
            evaluated += 1;
    ...
    atomic_write(&source_path, source.as_bytes())?;
    Ok(MutationReport {
        initial_passed: initial,
        final_passed: score,
        total_cases: task.cases.len(),
...
fn parse_manifest(text: &str) -> Result<MutationTask, ExecError> {
    ...
    for raw in text.lines() {
        ...
        let (key, value) = line
            .split_once('=')
            .ok_or_else(|| ExecError::CodeDenied(format!("invalid goal line `{line}`")))?;
        match key.trim() {
            ...
            "max_candidates" => {
                max_candidates = value
                    .trim()
                    .parse()
                    .map_err(|_| ExecError::CodeDenied("invalid max_candidates".into()))?
            }
            "timeout_ms" => {
                timeout_ms = value
                    .trim()
                    .parse()
                    .map_err(|_| ExecError::CodeDenied("invalid timeout_ms".into()))?
            }
            "case" => {
                ...
                if name.is_empty() {
                    return Err(ExecError::CodeDenied("goal case requires name|input|expected".into()));
                }
    ...
    Ok(MutationTask {
        source: source.ok_or_else(|| ExecError::CodeDenied("goal requires source".into()))?,
        cases,
        max_candidates,
        timeout_ms,
    })
    ...
    for case in cases {
        ...
        loop {
            if let Some(status) = child.try_wait()? {
                ...
                if status.success() && String::from_utf8_lossy(&output.stdout).trim() == case.expected.trim() {
                    passed += 1;
                }
                break;
            }
