...
const FAMILIES: &[Family] = &[
    ...
    Family {
        ...
        initial_interventions: 2,
        slow: &[
            "files|open|input.csv|",
            "spreadsheet|import|input.csv|",
            "spreadsheet|select|data|",
            "spreadsheet|summarize|data|",
            "files|save|report.csv|",
            "notes|write|report|complete",
        ],
        efficient: &[
            "spreadsheet|import|input.csv|",
            "spreadsheet|summarize|data|",
...
pub fn run(out: &Path) -> Result<SuiteReport, Box<dyn std::error::Error>> {
    ...
    let report = SuiteReport {
        ...
        observations: learner.metrics.watched_tasks,
        certified,
        initial_actions,
        certified_actions,
        initial_ms,
...
fn write_trace(
    ...
    for action in actions {
        trace.push_str(&format!("action={action}\n"));
    }
