...
[[maybe_unused]] static int run_week(const fs::path& out){
...
namespace mature {
using sd::HybridPlanner;
struct ME {Episode ep;int day{};std::string name;double manual{};bool scheduled{true};};
static void add(std::vector<ME>&v,size_t&i,int day,const std::string&dn,std::string task,Seq ops,double manual,std::string src="inputs/report_0.csv",std::string url="/news_0.html"){
 ++i;Episode e{i,std::move(task),"mature_day_"+std::to_string(day),std::move(ops),std::move(src),"outputs/mature_"+std::to_string(day)+"_"+std::to_string(i)+".txt",std::move(url),false};v.push_back({std::move(e),day,dn,manual,true});}
static std::vector<ME> workload(){
 std::vector<ME>v;size_t i=0;auto m=sd::motifs();const std::array<std::string,7>dn={"Mon Aug 24","Tue Aug 25","Wed Aug 26","Thu Aug 27","Fri Aug 28","Sat Aug 29","Sun Aug 30"};
 Seq morning=seq({"http.request","transform.extract","ipc.request","transform.summarize","state.write","notify.send"});
 Seq reports=seq({"fs.read","transform.extract","transform.summarize","ipc.request","fs.write","notify.send"});
 Seq health=seq({"proc.observe","transform.extract","transform.summarize","state.write","notify.send"});
 Seq backup=seq({"process.run","transform.summarize","fs.write","notify.send"});
 Seq sync=seq({"fs.read","transform.summarize","ipc.request","state.write","notify.send"});
 Seq reminders=seq({"timer.observe","state.read","transform.summarize","state.write","notify.send"});
 Seq digest=seq({"fs.read","transform.extract","transform.summarize","ipc.request","fs.write","notify.send"});
 for(int d=0;d<5;++d){
   add(v,i,d,dn[d],"morning_digest",morning,12,"inputs/report_0.csv","/news_"+std::to_string((d+3)%8)+".html");
   add(v,i,d,dn[d],"report_intake",reports,8,"inputs/report_"+std::to_string((d+2)%16)+".csv");
   add(v,i,d,dn[d],"system_health",health,4);
   add(v,i,d,dn[d],"local_sync",sync,3,"inputs/report_"+std::to_string((d+7)%16)+".csv");
   add(v,i,d,dn[d],"backup_verify",backup,3,"inputs/report_"+std::to_string((d+10)%16)+".csv");
   add(v,i,d,dn[d],"reminder_refresh",reminders,2);
   // Three project jobs each day; names are new, structure drawn from the learned family.
   for(int j=0;j<3;++j){int n=d*3+j;Seq q=m[(n*5+2)%m.size()];add(v,i,d,dn[d],"wk2_project_"+std::to_string(n),q,6+(n%4),"inputs/report_"+std::to_string((n+4)%16)+".csv","/news_"+std::to_string(n%8)+".html");}
   add(v,i,d,dn[d],"closeout",digest,6,"inputs/report_"+std::to_string((d+12)%16)+".csv");
 }
 add(v,i,5,dn[5],"morning_digest",morning,8,"inputs/report_0.csv","/news_1.html");add(v,i,5,dn[5],"system_health",health,3);add(v,i,5,dn[5],"backup_verify",backup,3,"inputs/report_11.csv");add(v,i,5,dn[5],"weekend_cleanup",m[4],6,"inputs/report_12.csv");
 add(v,i,6,dn[6],"morning_digest",morning,8,"inputs/report_0.csv","/news_2.html");add(v,i,6,dn[6],"system_health",health,3);add(v,i,6,dn[6],"backup_verify",backup,3,"inputs/report_13.csv");add(v,i,6,dn[6],"weekly_personal_summary",digest,10,"inputs/report_14.csv");add(v,i,6,dn[6],"next_week_prepare",m[11],8,"inputs/report_15.csv");add(v,i,6,dn[6],"reminder_refresh",reminders,2);
 return v;
}
struct DM{std::string name;size_t ep{},ok{},sem{},first{},firstsem{},firstone{},structural{},repairs{},rec{};double plan{},cons{},exec{},wall{},manual{},attn{};std::vector<double>pv;size_t perm_start{},perm_end{},prov_start{},prov_end{};};
static int run(const fs::path&out){fs::remove_all(out);fs::create_directories(out);auto work=out/"workspace";make_fixtures(work);TcpServer tcp;UnixServer uds(work/"drm.sock");LiveExecutor ex(work,tcp.port,work/"drm.sock");HybridPlanner drm;drm.provisional_cap=20;
 // Cold-start week is training history; same real execution, excluded from mature-week metrics.
 for(const auto&we:week::weekly_workload()){auto pm=drm.plan(we.ep);std::string err;if(!ex.execute(we.ep,err))return 3;drm.consolidate_pending();(void)pm;}
 auto eps=workload();std::array<DM,7>d;for(int x=0;x<7;++x){d[x].perm_start=drm.vocab.derived.size();d[x].prov_start=drm.provisional.size();}std::set<std::string>seen; // first-seen relative to mature week
 Baseline stateless;stateless.kind="stateless";Baseline cache;cache.kind="template_cache";Baseline check;check.kind="checkpoint_replay"; // pretrain exact baselines on week1 too
 for(const auto&we:week::weekly_workload()){stateless.plan(we.ep);cache.plan(we.ep);check.plan(we.ep);}size_t bs=0,bc=0,bk=0;double man=0,att=0;size_t sem=0,ok=0;
 std::ofstream tr(out/"mature_trace.csv");tr<<"episode,day,day_name,task,first_seen,semantic,planner_ms,consolidation_ms,executor_ms,attention_seconds,permanent_words,provisional_words,structural_change\n";
 for(const auto&me:eps){auto&x=d[me.day];if(x.name.empty())x.name=me.name;x.ep++;bool first=!seen.contains(me.ep.task);if(first){seen.insert(me.ep.task);x.first++;}auto p0=Clock::now();auto pm=drm.plan(me.ep);auto p1=Clock::now();std::string err;auto e0=Clock::now();bool suc=ex.execute(me.ep,err);auto e1=Clock::now();auto c0=Clock::now();size_t pc=drm.consolidate_pending();auto c1=Clock::now();pm.structural_change+=pc;double pl=std::chrono::duration<double,std::milli>(p1-p0).count(),co=std::chrono::duration<double,std::milli>(c1-c0).count(),ee=std::chrono::duration<double,std::milli>(e1-e0).count();double at=week::attention_seconds(first,pm,true);x.ok+=suc;x.sem+=pm.semantic;x.structural+=pm.structural_change;x.repairs+=pm.local_repair;x.rec+=pm.recovery;x.plan+=pl;x.cons+=co;x.exec+=ee;x.wall+=pl+co+ee;x.manual+=me.manual;x.attn+=at;x.pv.push_back(pl);if(first){x.firstsem+=pm.semantic;if(pm.semantic==1)x.firstone++;}x.perm_end=drm.vocab.derived.size();x.prov_end=drm.provisional.size();ok+=suc;sem+=pm.semantic;man+=me.manual;att+=at;bs+=stateless.plan(me.ep).semantic;bc+=cache.plan(me.ep).semantic;bk+=check.plan(me.ep).semantic;tr<<me.ep.idx<<','<<me.day<<','<<me.name<<','<<me.ep.task<<','<<first<<','<<pm.semantic<<','<<pl<<','<<co<<','<<ee<<','<<at<<','<<drm.vocab.derived.size()<<','<<drm.provisional.size()<<','<<pm.structural_change<<"\n";}
 std::ofstream dc(out/"mature_day_metrics.csv");dc<<"day,day_name,episodes,semantic_mean,first_seen,first_seen_semantic_mean,first_seen_one_decision_rate,planner_p95_ms,permanent_start,permanent_end,provisional_start,provisional_end,manual_minutes_modeled,attention_minutes_modeled,minutes_returned_modeled,attention_leverage\n";for(int q=0;q<7;++q){auto&x=d[q];double am=x.attn/60,ret=x.manual-am;dc<<q<<','<<x.name<<','<<x.ep<<','<<double(x.sem)/x.ep<<','<<x.first<<','<<(x.first?double(x.firstsem)/x.first:0)<<','<<(x.first?double(x.firstone)/x.first:0)<<','<<sd::percentile(x.pv,.95)<<','<<x.perm_start<<','<<x.perm_end<<','<<x.prov_start<<','<<x.prov_end<<','<<x.manual<<','<<am<<','<<ret<<','<<(am?x.manual/am:0)<<"\n";}
 double amin=att/60,ret=man-amin;std::ofstream js(out/"mature_summary.json");js<<std::fixed<<std::setprecision(6)<<"{\n  \"episodes\": "<<eps.size()<<",\n  \"success_rate\": "<<double(ok)/eps.size()<<",\n  \"semantic_total\": "<<sem<<",\n  \"semantic_mean\": "<<double(sem)/eps.size()<<",\n  \"stateless_semantic\": "<<bs<<",\n  \"template_semantic\": "<<bc<<",\n  \"checkpoint_semantic\": "<<bk<<",\n  \"permanent_words_start\": 9,\n  \"permanent_words_final\": "<<drm.vocab.derived.size()<<",\n  \"provisional_words_final\": "<<drm.provisional.size()<<",\n  \"modeled_manual_minutes\": "<<man<<",\n  \"modeled_attention_minutes\": "<<amin<<",\n  \"modeled_minutes_returned\": "<<ret<<",\n  \"attention_leverage\": "<<(amin?man/amin:0)<<",\n  \"uniform_vocabulary\": "<<(drm.vocab.audit()?"true":"false")<<"\n}\n";std::cout<<"mature episodes="<<eps.size()<<" success="<<ok<<" semantic="<<sem<<" permanent="<<drm.vocab.derived.size()<<" manual="<<man<<" attention="<<amin<<" returned="<<ret<<"\n";return ok==eps.size()&&drm.vocab.audit()?0:2;}
} // namespace mature
int main(int argc,char**argv){try{fs::path out="mature_results";for(int i=1;i+1<argc;++i)if(std::string(argv[i])=="--out")out=argv[i+1];return mature::run(out);}catch(const std::exception&e){std::cerr<<"fatal: "<<e.what()<<"\n";return 1;}}
