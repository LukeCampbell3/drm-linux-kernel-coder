...
# systemd reaches multi-user.target, and drmd.service starts. This checks
# systemd's own "Started drmd.service" status line on the console rather
# than drmd's own "listening on" stderr line -- under systemd, a service's
# stdout/stderr goes to the journal by default, not the console, so the
# latter never appears in the serial log even on a fully successful boot.
#
...
while [ $SECONDS -lt $deadline ]; do
  if [ -f "$SERIAL_LOG" ] && grep -aqE "Started.*drmd\.service" "$SERIAL_LOG" 2>/dev/null; then
    found=1
    break
  fi
...
if [ "$found" = "1" ]; then
  echo "PASS: systemd started drmd.service within ${TIMEOUT}s" >&2
  exit 0
else
  echo "FAIL: did not observe drmd.service starting within ${TIMEOUT}s -- last serial output:" >&2
